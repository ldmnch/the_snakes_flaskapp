...
# MAX_LEADERBOARD_ENTRIES will now primarily be for display guidance, not storage limit
# We can keep it if the client-side display logic for "All Scores" uses it,
# but the save_leaderboard function will not use it to trim the saved file.
app.config['MAX_LEADERBOARD_ENTRIES_DISPLAY'] = 10 # New config for display, if needed
...
def save_leaderboard(scores: list) -> bool: # MODIFIED
    ...
    try:
        # Sort all scores by time. This is good for consistent ordering in the raw file.
        scores.sort(key=lambda item: item.get('time', float('inf')))
        
        # DO NOT TRIM the 'scores' list before saving. Save all of them.
        # The old trimming logic:
        # max_entries = app.config['MAX_LEADERBOARD_ENTRIES']
        # trimmed_scores = scores[:max_entries]
        # json.dump(trimmed_scores, f, indent=4)
        # NOW:
        with open(temp_file, 'w') as f:
            json.dump(scores, f, indent=4) # Save the full list of scores

        os.replace(temp_file, leaderboard_file)
        logger.info(f"Leaderboard with {len(scores)} scores saved successfully to {leaderboard_file}")
        return True
...
# --- Routes (generate_maze_api, home, maze remain the same) ---
@app.route("/")
@app.route("/home")
def home():
...
def add_score_api():
    ...
    scores = load_leaderboard() # Load all existing scores
    scores.append({ # Add the new score
        "name": name, "time": time, "dimension": dimension,
        "timestamp": datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
    })

    if save_leaderboard(scores): # Save all scores (save_leaderboard now saves all)
...
def get_leaderboard_api():
    ...
    scores = load_leaderboard() # This will now return ALL scores from the file
    # Client-side will be responsible for filtering and displaying top N per category
